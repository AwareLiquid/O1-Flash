...
## 7. 实测发现：联合多问题训练 → 混合读出解决（2026-09-18）

训练参考的实测矩阵（合成票分类，600 步，同一批 heads/loss）：

| 实验 | dept acc | sev acc | urgent acc |
|---|---|---|---|
| 单问题训练（各自） | **1.00** | — | — |
| 联合 3 问题（纯液态读出） | 0.19-0.31 | 0.5-0.68 | ~1.0 |
| 联合 3 问题（纯 MLP 基线替换液态核） | 0.69 | 0.75 | 1.0 |
| **联合 3 问题（混合读出：液态 + 原始嵌入）** | **0.73** | **0.81** | **1.0** |

**结论链**：
1. **padding 污染**（已修）：可变长 pad token 污染扫描状态 →
   屏蔽扫描（masked scan）后单问题信号任意位置学满
2. **时间衰减偏置**（结构属性）：末位读出对早期 token 衰减 → 双向扫描缓解
3. **联合训练干扰的根因**：液态扫描把位置内容在时间上混合（这是 O(1)
   压缩记忆的代价）——纯位置读出无法再分离多信号。位置注意力在联合
   训练中无法锐化（实测权重 ~均匀）
4. **解法：混合读出**（`hybrid_readout=True`，默认开）——把**原始字节
   嵌入**并接在液态输出后面，决策头获得直接的 token 级精确访问。
   这正是 M1 仓库 HOLA 式设计族验证过的"压缩状态 + 精确召回"混合：
   液态状态管记忆（O(1)、跨窗），原始嵌入管内容匹配

**注意**：混合读出的原始嵌入并接只用于**读出**（prefill 时在显存中），
O(1) 携带状态仍是纯液态——流式/端侧的内存主张不变。

## 8. 读出机制定型：mean-pool 选项匹配（2026-09-20）

A100 诊断（HANDOFF §2b）锁定：信号损失在读出对齐，线性头天花板
1.0/1.0/1.0。本仓的变体对比（联合合成基准，600 步 tiny）：

| 读出变体 | dept | sev | urgent |
|---|---|---|---|
| 每类线性头（共享 score_proj 基线） | 0.547 | 0.742 | 1.0 |
| logsumexp 选项匹配（旧） | 0.730 | 0.922 | 1.0 |
| max-pool 选项匹配 | 0.453 | 0.609 | 1.0 |
| **mean-pool 选项匹配（现用）** | **0.977** | **0.973** | **1.0** |

**机制解释**：峰选择（logsumexp/max）被干扰短语制造的伪峰欺骗；
均匀 mean-pool 与线性头天花板的"均匀位置混合"同构，同时保留
选项式（schema 有界）打分。概率头同样从 logsumexp 改为 mean-pool
（default 配置下 urgent 从塌缩到基础率 0.54 → 0.992）。

**default 配置（d256/4L，4.53M）联合训练 600 步实测**：
dept 1.000 / sev 1.000 / urgent 0.992，loss 3.5→1.9。
