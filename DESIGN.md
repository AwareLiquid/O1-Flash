...
## 7. 实测发现：联合多问题训练是液态核的开放问题（2026-09-18）

训练参考的实测矩阵（合成票分类，600-800 步，同一批 heads/loss）：

| 实验 | dept acc | sev acc | urgent acc |
|---|---|---|---|
| 单问题训练（各自） | **1.00** | — | — |
| 联合 3 问题（液态核） | 0.19-0.31 | 0.5-0.68 | ~1.0 |
| 联合 3 问题（纯 MLP 基线替换液态核） | **0.69** | **0.75** | 1.0 |

**结论链**：
1. **padding 污染**（已修）：可变长 pad token 以不可预测数量污染扫描状态，
   屏蔽扫描（masked scan）后单问题信号在任意位置可学满
2. **时间衰减偏置**（结构属性）：末位读出对早期 token 衰减——已用双向
   扫描 + 每问题位置注意力缓解，但联合多问题仍失败
3. **多任务联合训练的干扰**：液态核的时序混合使共享表示难以同时承载
   多个决策信号；纯 MLP 基线在同一预算下显著更好。位置注意力的
   query-key 对齐在联合训练中无法锐化（实测注意力权重 ~均匀）

**当前立场（诚实）**：
- 训练参考按**单问题训练 + 并行推理**组织（推理并行性是结构保证，
  与训练方式无关）
- 联合多问题训练 = 本仓头号开放问题；下一步候选：每问题独立读出通道
  （分开的 encoder 分支）、对比损失、或 Jev 式 listwise option-slot 输入
- 不做任何掩盖：以上数字全部可复现（`benchmarks/decision_bench.py`）
